...
pub fn solution_part_1(_n: u32) -> () {
    ...
    content.lines().for_each(|line| {
        ...
        // postMessageToWorker(show_message, &format!("characters: {:?}", characters));
        ...
        for (_index, character) in characters.iter().enumerate() {
            if is_digit_regex.is_match(character.to_string().as_str()) {
                // postMessageToWorker(show_message, &format!("index: {}, character: {}", index, character));
                if first_digit == None {
        ...
        // postMessageToWorker(show_message, &format!("first_digit: {:?}, last_digit: {:?}", first_digit, last_digit));
