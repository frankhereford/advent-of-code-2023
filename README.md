...
* The rust WASM code is run in a web-worker to avoid blocking the main JS event loop, and which communicate to the main components through web-worker-like pub/sub methods. This architecture frees up from needing to write async code, and we can plow ahead functionally without impact on the UI/UX.
